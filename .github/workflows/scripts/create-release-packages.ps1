...
<#
.SYNOPSIS
    Build Grove template release archives for each supported AI assistant and script type.
...
function Rewrite-Paths {
    ...
    $Content = $Content -replace '(/?)\bmemory/', '.grove/memory/'
    $Content = $Content -replace '(/?)\bscripts/', '.grove/scripts/'
    $Content = $Content -replace '(/?)\btemplates/', '.grove/templates/'
    return $Content
}
...
function Generate-Commands {
    ...
    foreach ($template in $templates) {
        ...
        $outputFile = Join-Path $OutputDir "grove.$name.$Extension"
...
function Generate-CopilotPrompts {
    ...
    $agentFiles = Get-ChildItem -Path "$AgentsDir/grove.*.agent.md" -File -ErrorAction SilentlyContinue
...
function Build-Variant {
    ...
    $specDir = Join-Path $baseDir ".grove"
    New-Item -ItemType Directory -Path $specDir -Force | Out-Null
    
    # Copy memory directory
    if (Test-Path "memory") {
        Copy-Item -Path "memory" -Destination $specDir -Recurse -Force
        Write-Host "Copied memory -> .grove"
    }
    ...
    if (Test-Path "templates") {
        ...
        Write-Host "Copied templates -> .grove/templates"
    }
    ...
    # Create zip archive
    $zipFile = Join-Path $GenReleasesDir "grove-template-${Agent}-${Script}-${Version}.zip"
...
Get-ChildItem -Path $GenReleasesDir -Filter "grove-template-*-${Version}.zip" | ForEach-Object {
    Write-Host "  $($_.Name)"
}