...
rewrite_paths() {
  sed -E \
    -e 's@(/?)memory/@.grove/memory/@g' \
    -e 's@(/?)scripts/@.grove/scripts/@g' \
    -e 's@(/?)templates/@.grove/templates/@g'
}
...
generate_copilot_prompts() {
  ...
  for agent_file in "$agents_dir"/grove.*.agent.md; do
...
build_variant() {
  ...
  SPEC_DIR="$base_dir/.grove"
  mkdir -p "$SPEC_DIR"
  
  [[ -d memory ]] && { cp -r memory "$SPEC_DIR/"; echo "Copied memory -> .grove"; }
  ...
  if [[ -d scripts ]]; then
    ...
    case $script in
      sh)
        [[ -d scripts/bash ]] && { cp -r scripts/bash "$SPEC_DIR/scripts/"; echo "Copied scripts/bash -> .grove/scripts"; }
        # Copy any script files that aren't in variant-specific directories
        find scripts -maxdepth 1 -type f -exec cp {} "$SPEC_DIR/scripts/" \; 2>/dev/null || true
        ;;
      ps)
        [[ -d scripts/powershell ]] && { cp -r scripts/powershell "$SPEC_DIR/scripts/"; echo "Copied scripts/powershell -> .grove/scripts"; }
        # Copy any script files that aren't in variant-specific directories
        find scripts -maxdepth 1 -type f -exec cp {} "$SPEC_DIR/scripts/" \; 2>/dev/null || true
        ;;
    esac
  ...
  ( cd "$base_dir" && zip -r "../grove-template-${agent}-${script}-${NEW_VERSION}.zip" . )
  echo "Created $GENRELEASES_DIR/grove-template-${agent}-${script}-${NEW_VERSION}.zip"
}
...
ls -1 "$GENRELEASES_DIR"/grove-template-*-"${NEW_VERSION}".zip
